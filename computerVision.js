...
class LocalCache {
    ...
    get(key) {
        if (this.valueCache[key] == undefined) {
            if(debugAll == true){ console.log('adding to local cache', key, this.valueCache, this.valueCache[key], dw.get(key)) }
            this.valueCache[key] = dw.get(key) ?? null
        }
        return this.valueCache[key]
    }
    set(key, value) {
        if(debugAll == true){ console.log('setting cache value', key, value) }
